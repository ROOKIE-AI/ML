...
# plt_one_addpt_onclick 类用于创建一个交互式绘图
class plt_one_addpt_onclick:
    """ 用于运行一个交互式图 """
    def __init__(self, x, y, w, b, logistic=True):
        self.logistic = logistic  # 标记是否使用逻辑回归
        pos = y == 1  # 标签为1的位置
        neg = y == 0  # 标签为0的位置
        ...
        ax.scatter(x[pos], y[pos], marker='x', s=80, c='red', label="malignant")  # 绘制标签为1的点
        ax.scatter(x[neg], y[neg], marker='o', s=100, label="benign", facecolors='none', edgecolors=dlblue, lw=3)  # 绘制标签为0的点
        ...
        self.cid = fig.canvas.mpl_connect('button_press_event', self.add_data)  # 连接鼠标点击事件
        if self.logistic:
            self.bcalc = Button(axcalc, 'Run Logistic Regression (click)', color=dlblue)
            self.bcalc.on_clicked(self.calc_logistic)  # 绑定逻辑回归按钮点击事件
        else:
            self.bcalc = Button(axcalc, 'Run Linear Regression (click)', color=dlblue)
            self.bcalc.on_clicked(self.calc_linear)  # 绑定线性回归按钮点击事件
        self.bthresh = CheckButtons(axthresh, ('Toggle 0.5 threshold (after regression)',))
        self.bthresh.on_clicked(self.thresh)  # 绑定阈值切换按钮点击事件
        self.resize_sq(self.bthresh)

    def add_data(self, event):
        # 在图表上添加数据点
        if event.inaxes == self.ax[0]:
            x_coord = event.xdata   # 获取鼠标点击的x坐标
            y_coord = event.ydata   # 获取鼠标点击的y坐标
